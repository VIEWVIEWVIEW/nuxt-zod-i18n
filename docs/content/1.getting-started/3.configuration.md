---
title: Configuration
description: Nuxt zodI18n configuration guide
navigation:
  icon: i-lucide-sliders
---

You can configure  Nuxt zodI18n with the `zodI18n` property in your `nuxt.config` file.

```ts [nuxt.config.ts]
export default defineNuxtConfig({
  zodI18n: {
    // My custom configuration
  }
})
```

## `dateFormat`

- Type: [Intl.DateTimeFormatOptions](https://developer.mozilla.org/fr/docs/Web/JavaScript/Reference/Global_Objects/Intl/DateTimeFormat/DateTimeFormat){:target="_blank"}
- Default: `{ day: 'numeric', month: 'long', year: 'numeric'}`

Change default date format in error message linked to `Date`.

```ts [nuxt.config.ts]
export default defineNuxtConfig({
  zodI18n: {
    dateFormat: {
      day: '2-digit',
      month: 'numeric',
      year: 'numeric'
    }
  }
})
```

::note
As result show 20/12/2020 instead of 20 December 2020
::

## Locale loading

Zod v4 now bundles the full set of error translations. The module automatically applies the closest Zod locale for the active Nuxt I18n language at startup and whenever the locale changes. No additional configuration is required.

If you need to adjust wording, you can still override any message by defining the `zodI18n` namespace inside your own locale files.
